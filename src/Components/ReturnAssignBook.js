...
import { returnAssignBook } from '../Redux/ReturnAssignBook/ReturnAssignBookActions';
import { fetchAllBook } from '../Redux/AllBook/AllBookActions'


function ReturnAssignBook(){
  // store data access start
  const returnAssignBookData = useSelector(state =>state.ReturnAssignBook)
...
// when allBookResult data change than data add into constant,hook End
   ...
   // add data of addExam api into constant,hook start
      useMemo(()=>{
        if(returnAssignBookData && returnAssignBookData.return_assign_book && returnAssignBookData.return_assign_book.error){
          setError(returnAssignBookData.return_assign_book.error)
        }
      },[returnAssignBookData])
   // add data of addExam api into constant,hook End

   // add data of addExam api into constant,hook start
      useMemo(()=>{
    if(returnAssignBookData && returnAssignBookData.return_assign_book && returnAssignBookData.return_assign_book.success){
           setSuccessStatus(returnAssignBookData.return_assign_book.success)
        }
      },[returnAssignBookData])
   // add data of addExam api into constant,hook End
  ...
  const callbackFunction = (childData) => {
    ...
    const returnAssignBookHandler = (event) =>{
    event.preventDefault()
    const return_assign_book_info = inputValues;
      dispatch(returnAssignBook(return_assign_book_info))
  }
       ...
       <Header parentCallback = {() =>callbackFunction()}/>
        ...
        <div className="dashboard-page-one">
          ...
          <div className="dashboard-content-one">
            ...
            {returnAssignBookData && returnAssignBookData.return_assign_book_loading === false ? successStatus === false || successStatus === null  ? (
              <div className="card-body">
                ...
                <form className="new-added-form" onSubmit={(e) =>returnAssignBookHandler(e)}>
                  <div className="row">
...
export default ReturnAssignBook;
